...
UINT App_ThreadX_Init(VOID *memory_ptr)
{
  ...
  if (tx_thread_create(&thread_stationapp, "RoboRally App", Station_thread_entry, 0, pointer,
  ...
  /* Create Semaphore.  */
  if (tx_semaphore_create(&semaphore_ledblink, "LED Blink", 0) != TX_SUCCESS)
  {
  ...
  if (tx_semaphore_create(&semaphore_photocell, "photocell", 0) != TX_SUCCESS)
  {
    return TX_SEMAPHORE_ERROR;
  }
